...
def eatApple(apple, score):
    apple = [random.randrange(1,size*square)*square,random.randrange(1,size*square)*square]
    score += 1
    return apple, score
...
apple =[np.random.randint(0, size) * square, np.random.randint(0, size) * square]
score = 0
prevButton = 1
curDir = 1
...
while True:
    ...
    for position in snakeBody:
        cv2.rectangle(img,(position[0],position[1]),(position[0]+square,position[1]+square),(0,255,0),3)
    ...
    if wallCollide(snakeHead) == 1 or touchBody(snakeBody) == 1:
